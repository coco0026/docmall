...
SELECT  C1.COMMON_CODE,
        C1.COMMON_CODE_NM,
        C2.COMMON_CODE_CHILD_NM
FROM COMMON_CODE_TBL C1, COMMON_CODE_DETAIL_TBL C2
WHERE C1.COMMON_CODE = C2.COMMON_CODE AND
      C1.COMMON_CODE like 'C%';
...
WHERE A.GDS_CODE = B.GDS_CODE AND
      A.CATE_CODE = C.COMMON_CODE
GROUP BY C.COMMON_CODE_NM
ORDER BY C.COMMON_CODE_NM;



SELECT C1.COMMON_CODE_NM,
       SUM(B.ORDER_DTL_AMT)
FROM GOODS_TBL A, 
     ORDER_DETAIL_TBL B, 
     COMMON_CODE_TBL C1, 
     COMMON_CODE_DETAIL_TBL C2
WHERE A.GDS_CODE = B.GDS_CODE AND
      A.CATE_CODE = C1.COMMON_CODE AND
      C1.COMMON_CODE = C2.COMMON_CODE
GROUP BY C1.COMMON_CODE_NM
ORDER BY C1.COMMON_CODE_NM;



SELECT COMMON_CODE_NM, 
       SUM(c.order_dtl_amt)
FROM COMMON_CODE_TBL A, GOODS_TBL B, ORDER_DETAIL_TBL C
WHERE COMMON_CODE like 'C%' AND
      B.CATE_CODE = A.COMMON_CODE AND
      B.GDS_CODE = C.GDS_CODE
GROUP BY COMMON_CODE_NM;


SELECT COMMON_CODE_NM AS primary_cd, SUM(c.order_dtl_amt) AS sales_p FROM COMMON_CODE_TBL A, 
GOODS_TBL B, ORDER_DETAIL_TBL C WHERE COMMON_CODE like 'C%' AND B.CATE_CODE = A.COMMON_CODE 
AND B.GDS_CODE = C.GDS_CODE GROUP BY A.COMMON_CODE_NM ORDER BY A.COMMON_CODE_NM ;
