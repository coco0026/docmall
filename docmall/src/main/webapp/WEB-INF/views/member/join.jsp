	...
	//html 문서와 내용을 브러우저가 읽고 난 이후 동작되는 특징
		$(document).ready(function(){
			...
			$("#btnJoin").on("click", function(){
				...
				}else if(mbr_pw.search(num) < 0 || mbr_pw.search(eng) < 0 || mbr_pw.search(spe) < 0 ){
				...
				//휴대폰번호 유효성검사
				let phonenum = $('#mbr_telno').val();
				let regPhone = /(01[0|1|6|9|7])[-](\d{3}|\d{4})[-](\d{4}$)/g;
				if(!regPhone.test(phonenum)){
					alert('잘못된 휴대폰 번호입니다. 하이픈기호(-)를 추가하여 형식에 맞게 작성해주세요.');
					$('#mbr_telno').focus();
					return false;    
				}
				...
				//주소 유효성검사 Start------------------------
				let mbrZip = $('input[name=mbr_zip]').val()
				let mbr_daddr = $('input[name=mbr_daddr]').val()
				if(mbrZip == "") {
					alert("우편번호를 입력하세요");
					return;
				}
